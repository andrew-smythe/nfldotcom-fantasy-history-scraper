...
module.exports = {
    get: async function(page, year) {
        ...
        while (attempts < 10) {
            try {
                await page.waitForSelector('table.tableType-team', { visible: true, timeout: 5000 });
                break;
            } catch (e) { 
                attempts++;
                if (attempts == 10) {
                    console.error('ERROR - 10 attempts made. Carrying on.', e);
                }
            }
        }
    ...
    downloadPlayerStats: async function (db, page, baseUrl, rosterPositions, year, week, teamId) {
        ...
        /*if (year < 2013 || (year == 2013 && teamId < 8) || (year == 2013 && teamId == 8 && week < 3)) {
            return;
        }
        if (year != 2013) {
            return;
        ...
        while (attempts < 10) {
            try {
                ...
                attempts++;
                if (attempts == 10) {
                    console.error('ERROR - 10 goto attempts made. Carrying on.');
                }
            }
        }
        ...
        try {
            ...
            while (attempts < 10) {
                try {
                    await page.waitForSelector('td.playerNameAndInfo a', { visible: true, timeout: 5000 });
                    break;
                } catch (e) { 
                    attempts++;
                    if (attempts == 10) {
            ...
            let players = await page.evaluate(() => {
                ...
                return playerElements.map((team, i) => ({
                    ...
                    rosterPosition: team.querySelector('td.teamPosition span').childElementCount === 0 ? team.querySelector('td.teamPosition span').innerHTML : team.querySelector('td.teamPosition span em').innerText,
                    ...
                    if (rosterPositionId) {
                        ...
                        try {
                            ...
                            let result = await db.query('\
                                ...
                                fumbles, twoPoints, \
                                pats, nineteenFgs, twentynineFgs, thirtynineFgs, fourtynineFgs, fiftyFgs, \
                                ...
                                + ', ' + players[i].pats + ', ' + players[i].nineteenFgs + ', ' + players[i].twentynineFgs + ', ' + players[i].thirtynineFgs + ', ' + players[i].fourtynineFgs + ', ' + players[i].fiftyFgs
