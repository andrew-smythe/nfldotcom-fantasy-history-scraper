                ...
                try {
                    ...
                    try {
                        ...
                        for (let t of trades) {
                            ...
                            if (!team1Id) {
                                throw error('Could not find team for NFL ID ' + t.team1NflId);
                            }

                            let team2Id = teams.find(e => e.nflId === t.team2NflId && e.year === year)?.id;
                            if (!team2Id) {
                                throw error('Could not find team for NFL ID ' + t.team2NflId);
                            }
                            ...
                            try {
                                ...
                                for (let p of t.players) {
                                    ...
                                    if (!originalTeamId) {
                                        throw error('Could not find team for player ' + p.name + ' from team with NFL ID ' + p.originalTeamNflId);
                                    }
